...
model_features = model_meta['features']
print(f"Model features: {model_features}")

# Load system costs data for recommendations
system_costs_path = os.path.join(data_dir, 'system_costs.csv')
system_costs = pd.read_csv(system_costs_path)

# Load soil pH ranges for crop compatibility
...
def _normalize_market_price_rows(payload):
    if isinstance(payload, dict):
        if isinstance(payload.get("market_data"), list):
...
async def get_market_prices_from_market_endpoint(crop=None, location=None):
    try:
        ...
        return _normalize_market_price_rows(message.content.get("prices", []))
    except Exception as exc:
        print(f"Error fetching live market prices: {exc}")
        return []


# Get market prices for crops
async def get_market_prices(crops, location, district=None, prices_data=None):
    if not crops:
        return []

    preferred_location = location or district or "Baramati"
    preferred_location = str(preferred_location).strip() or "Baramati"
    ...
    try:
        ...
        for crop in crops:
            if not crop:
                continue
            crop_prices = await get_market_prices_from_market_endpoint(str(crop), preferred_location)
            if crop_prices:
                live_prices.extend(crop_prices)

        if live_prices:
            normalized_location = re.sub(r'[^a-z0-9]+', ' ', preferred_location.lower()).strip()

            def location_score(item):
                row_location = re.sub(r'[^a-z0-9]+', ' ', str(item.get('location') or '').lower()).strip()
                if not normalized_location:
                    return 0
                if normalized_location in row_location:
                    return 5
                if any(token in row_location for token in normalized_location.split() if len(token) > 2):
                    return 3
                return 0

            scored = []
            for item in live_prices:
                score = location_score(item)
                if score > 0:
                    scored.append((score, item))
            if scored:
                scored.sort(key=lambda x: (-x[0], x[1].get('price_per_quintal', 0)))
                return [
                    {
                        "location": item.get("location"),
                        "price_per_quintal": item.get("price_per_quintal") or item.get("price_per_tonne"),
                        "date": item.get("date"),
                    }
...
async def recommend_system(data: RecommendationRequest):
    try:
        ...
        for _, system in suitable_systems.iterrows():
            ...
            # Apply model prediction for ROI if available features match
            try:
                input_features = {
                ...
                # Check if we have necessary features for prediction
